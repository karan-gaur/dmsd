...
FOR EACH ROW BEGIN
UPDATE DOC_COPY SET Avaialble=FALSE WHERE UUID=NEW.Doc_Detail;
END ;

-- Add Trigger to update Document Availability on Reserve
CREATE TRIGGER BOOK_AVAILABLE
AFTER UPDATE ON RESERVES
FOR EACH ROW BEGIN
    IF (NEW.RStatus IS TRUE) THEN
        UPDATE DOC_COPY SET Avaialble=TRUE WHERE UUID=NEW.Doc_Detail;
    END IF;
END ;
...
-- Add Trigger to make  _COPY available on document return
CREATE TRIGGER BOOK_AVAILABLE
